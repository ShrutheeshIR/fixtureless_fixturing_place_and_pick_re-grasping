...
class Fetch_Robot():
    ...
    def get_ik_from_jointtrajectory(self, js):
        joint_names = js.joint_names
        # joint_values = js.points[0].positions
        joint_values = []
        for name in js.joint_names:
            listindex = self.robot.get_current_state().joint_state.name.index(name)
            joint_values.append(self.robot.get_current_state().joint_state.position[listindex])

        current_robot_state = moveit_msgs.msg.RobotState()
        current_robot_state.joint_state.header.stamp = rospy.Time.now()
        current_robot_state.joint_state.position = joint_values
        current_robot_state.joint_state.name = joint_names
    ...
    def addCollisionBoundingBox(self, name, position, orientation, dimensions):
        ...
        self.scene.add_box(name, object_pose, tuple(dimensions))
        start = rospy.get_time()
        second = rospy.get_time()
        while (second - start) < self.timeout and not rospy.is_shutdown():
            if not name in self.scene.get_known_object_names():
                break
            second = rospy.get_time()

    

    def addCollisionObject(self, objectname, transfrom_Q, filename, size_scale = 0.001):
        ...
        # rotation_y = tf.transformations.quaternion_from_euler(0, 0, 0)
        # r = tf.transformations.quaternion_multiply(rotation_y, r)
        ...
        print("Adding mesh of : ", filename)
        print("Obj pose is : ", object_pose)
    ...
    def planFollowEndEffectorTrajectory(self, initial_robot_state, trajectory, gap_dis, jump):
        current_endeffector = self.group.get_end_effector_link()
        print("END EFFECTOR LINK!", current_endeffector)
        print("*************************")
        self.group.set_end_effector_link("gripper_link")
        waypoints = []
        for t in trajectory:
        ...
        (plan, fraction) = self.group.compute_cartesian_path(waypoints, gap_dis, jump, True, self.cons)
    ...
    def display_trajectory(self, plans, start_state = None):
        # print("DISPLAY PLANS ! ", type(plans))
        ...
        # for p in plans:
        display_trajectory.trajectory.append(plans)
        # Publish
