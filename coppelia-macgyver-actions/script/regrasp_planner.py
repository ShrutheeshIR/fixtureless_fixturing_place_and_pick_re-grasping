...
class RegripPlanner():
    ...
    def getAllGrasps(self):
        ...
        for grasppose, jawwidth in zip(self.freegriprotmats, self.freegripjawwidth): 
            ...
            grasppose = pandageom.cvtMat4(rm.rodrigues([0, 1, 0], 0)) * grasppose
    ...
    def getGraspsById(self, graspids):
        ...
        for i in graspids:
            ...
            grasppose = pandageom.cvtMat4(rm.rodrigues([0, 1, 0], 0)) * dc.strToMat4(result[0][0])
            # print("GRASPPOSE")
            # print(grasppose)
            # done
    ...
    def getPlacementIdFromPose(self, pose):
        ...
        """
        print("object pose : ", pose[:3, 3].squeeze(), R.from_dcm(pose[:3, :3]).as_euler('xyz', degrees=True))
        obj_dir_pos_2match = self.getGroundDirection(pose)
        print("GN Dir : ", obj_dir_pos_2match)
        diff = 2.0 
        ...
        for placementid, placementpose in zip(self.placementid, self.tpsmat4s):
            ...
            # print("PLACEMENTSSS!!")
            # print(placement_pose[:3, 3].squeeze(), R.from_dcm(placement_pose[:3, :3]).as_euler('xyz', degrees=True))
            # print("PLACEMENTS!")
            currnt_dir_pos = self.getGroundDirection(placement_pose)
            print("GN Dir : ", currnt_dir_pos)

            
            currnt_diff = np.linalg.norm(currnt_dir_pos - obj_dir_pos_2match)
            if currnt_diff <= diff:
                closest_placementid = placementid
                best_placement_pose = placement_pose
                diff = currnt_diff
        # print(best_placement_pose[:3, 3].squeeze(), R.from_dcm(best_placement_pose[:3, :3]).as_euler('xyz', degrees=True))
        # done
